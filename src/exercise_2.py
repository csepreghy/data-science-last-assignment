...
def exercise_2_init__():
  ...
  print('weights with all features: ', weights)
  print('total cost for all features:', cost)

  predictions = get_hypothesis(weights, X_test_std, X_test_std.shape[1] - 1)

  rmse_all_features = rmse(predictions, y_test)
  print('RMSE for all features: ', rmse_all_features)

  plotify.plot(cost, 
               title='Gradient descent showing how cost function converges',
  ...
  weights_fa, cost_fa = multivarlinreg(X_train_fa, y_train, alpha=0.001, n_iters=500)
  ...
  print('weights for first feature', weights_fa)

  plotify.plot(cost_fa,
               title='Gradient descent showing how cost function converges (one feature)',
               xlabel='Number of iterations',
               ylabel='Cost')
